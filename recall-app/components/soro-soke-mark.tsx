// Talking-drum icon mark from the brand kit (sorosoke-brand-kit/svg/
// sorosoke-icon-mark.svg), inlined so it ships with the bundle and scales
// crisply at any size. Sits beside the animated wordmark as a steady anchor
// while the letters resolve.

type Props = {
  /** Rendered height in px; width keeps the mark's 9:8 aspect ratio. */
  size?: number;
  className?: string;
};

export function SoroSokeMark({ size = 30, className }: Props) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="30 20 180 160"
      height={size}
      width={Math.round(size * 1.125)}
      aria-hidden="true"
      className={className}
    >
