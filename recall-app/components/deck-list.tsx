    ...
    <section className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Separate spaces for different parts of your vocabulary. One daily review brings them together.</p>
        </div>
          ...
          <Link key={deck.id} href={`/decks/${deck.id}`} className="group rounded-3xl border border-white/10 bg-white/5 p-5 transition hover:border-emerald-300/30 hover:bg-white/8">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400 line-clamp-2">{deck.description ?? "Words that share something."}</p>
              </div>
              {deck.dueCount > 0 ? (
